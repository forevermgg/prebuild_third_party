...
# 源代码和目标目录
readonly FMT_REPO_URL="https://github.com/fmtlib/fmt.git"
readonly FMT_SRC_DIR="fmt"
readonly PREFIX_DIR="./prefix/fmt"
readonly BUILD_DIR="build_fmt_android"
...
if [ ! -d "$FMT_SRC_DIR" ]; then
    print_info "Cloning fmt repository..."
    git clone --depth 1 --branch 12.1.0 "$FMT_REPO_URL" "$FMT_SRC_DIR"
else
    print_info "Updating existing fmt repository..."
    git -C "$FMT_SRC_DIR" pull origin 12.1.0
fi
...
if [ ! -f "$FMT_SRC_DIR/CMakeLists.txt" ]; then
    print_error "fmt source directory '$FMT_SRC_DIR' is invalid"
    exit 1
fi

print_info "--- Starting fmt Android Build (Shared Lib .so) ---"
...
for ABI in "${ABIS[@]}"; do
    print_info "=========================================="
    print_info "Building fmt for ABI: $ABI"
    print_info "=========================================="
    ...
    # 执行编译和安装
    print_info "Building and installing fmt..."
    cmake --build "$ABI_BUILD_DIR" --target install

    # 验证安装结果
    if [ ! -f "$PREFIX_DIR/$ABI/lib/libfmt.so" ]; then
        print_error "Build failed: libfmt.so not found for $ABI at $PREFIX_DIR/$ABI/lib/"
        exit 1
    fi

    print_success "Successfully built and installed fmt for $ABI"
done
...
cp -r "$PREFIX_DIR/${ABIS[0]}/include/fmt" "$PREFIX_DIR/include/"
...
print_info "--------------------------------------------------------"
print_success "fmt Android Build Completed Successfully for all target ABIs!"
